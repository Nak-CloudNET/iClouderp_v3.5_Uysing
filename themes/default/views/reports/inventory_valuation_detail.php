                        ...
                        <?php
							...
							foreach($prDetails as $pr)
							{
                                ...
                                if ($pr->tran_type == 'PURCHASE'
                                    || $pr->tran_type == 'SALE RETURN'
                                    || $pr->tran_type == 'OPENING QUANTITY')
								{
                                    $p_qty = $pr->quantity_balance_unit;
								}else if($pr->tran_type == 'TRANSFER') {
                                    $p_qty = $pr->quantity_balance_unit;
								}
								else if( $pr->tran_type == 'ADJUSTMENT') {
                                    $p_qty = $pr->quantity_balance_unit;
								}
								else if( $pr->tran_type == 'USING STOCK') {
                                    $p_qty = $pr->quantity_balance_unit;
								}
								else if( $pr->tran_type == 'RETURN USING STOCK') {
                                    $p_qty = $pr->quantity_balance_unit;
								}
								else if( $pr->tran_type == 'CONVERT') {
                                    $p_qty = $pr->quantity_balance_unit;
								}
								else if( $pr->tran_type == 'SALE' || $pr->tran_type == 'DELIVERY')
								{
									if($pr->quantity > 0) {
							...
							<tr>
								...
								<td class="text-right"><?= $this->erp->formatMoney($pr->avg_cost) ?></td>
